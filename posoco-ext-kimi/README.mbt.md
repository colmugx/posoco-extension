...
## kimi 独有 API（不进 ModelPort trait）

```moonbit
let port = @kimi.kimiModelPort::new(config)
...
```moonbit
let config = @kimi.kimiConfig::new(
  api_key="sk-...",
  model="kimi-v4-flash",
)
let port = @kimi.kimiModelPort::new(config)
...
Deprecated kimi aliases `kimi-chat` and `kimi-reasoner` are not
